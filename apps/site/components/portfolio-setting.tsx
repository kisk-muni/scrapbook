...
export default function PortfolioSettingCard({
  defaultName,
  updatePortfolio,
}: {
  defaultName: string;
  updatePortfolio: (args: { name: string }) => ServerActionResult<Profile>;
}) {
  const maxLength = 32;
  ...
  return (
    <SettingCard>
      <form
        ...
        onSubmit={(e) => {
          ...
          startSubmitTransition(async () => {
            ...
            const result = await updateUserName({ name: ref.current?.value });
      ...
      >
        <SettingHeader>
          <SettingHeadline>Portfolio</SettingHeadline>
          <SettingDescription>
